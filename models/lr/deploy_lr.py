...
logging.basicConfig(filename='/var/log/suricata/lr_model_binary_predictions.log', level=logging.INFO, format='%(asctime)s - %(message)s')

# Load the Logistic Regression model (not Decision Tree)
lr_model = joblib.load('/models/logistic_regression_binary_model.joblib')
...
def process_batch():
    ...
    if len(joblib_batch) > 0:
        joblib_predictions = lr_model.predict(joblib_batch)
