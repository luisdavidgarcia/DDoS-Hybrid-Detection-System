...
logging.basicConfig(filename='/var/log/suricata/rf_model_binary_predictions.log', level=logging.INFO, format='%(asctime)s - %(message)s')

# Load the Logistic Regression model (not Decision Tree)
rf_model = joblib.load('/models/random_forest_binary_model.joblib')
...
def process_batch():
    ...
    if len(joblib_batch) > 0:
        joblib_predictions = rf_model.predict(joblib_batch)
